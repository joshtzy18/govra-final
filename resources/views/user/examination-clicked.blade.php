...
<div class="h-auto w-full border-solid border-2  mb-5 flex flex-col border-red-500 announcement-container">
  <div class="h-auto w-full">
      <span class="flex flex-wrap text-start font-sans font-bold text-lg bg-red-600 text-white p-2 md:text-xl xl:text-2xl">{{$exam->title}}</span>
  </div>

  {{-- published text --}}
  <div class="h-auto w-auto mt-1 px-2 py-2">
    <span class="text-sm font-sans text-gray-400 sm:text-md md:text-[16px] xl:text-lg">Published: {{$exam->getFormattedDate()}}</span>
  </div>

  {{-- announce no text --}}
  <div class="flex h-auto w-full justify-end mt-1 px-2">
    <span class="font-sans text-sm sm:text-md md:text-[16px] xl:text-lg ">{{$exam->examination_advisory_no}}</span>
  </div>

  {{-- announcement text --}}
  <div class="flex h-auto w-full justify-center my-6">
    <span class="font-semibold font-sans md:text-lg xl:text-xl">ADVISORY</span>
  ...
  <div class="flex flex-col">
    <div class="flex flex-row w-full h-auto border-solid border-2 border-gray-50 ">
      <span class=" font-bold font-sans py-16 pl-16 w-[50%] xl:w-[38%] justify-end md:pl-36 lg:pl-[17rem]  lg:py-10 xl:pl-[23rem] xl:text-lg">TO:</span>
      <span class="flex flex-wrap px-5  text-justify font-sans font-semibold md:p-3 lg:p-2 xl:text-lg">{{$exam->to}}</span>
    </div>
  ...
  <div class="w-full h-auto flex flex-row items-center content-center bg-gray-100 py-2 md:justify-center md:items-center xl:justify-start xl:item">
    ...
    <i class="font-sans underline px-5 flex flex-wrap text-justify font-semibold xl:text-lg">{{$exam->subject}}</i>
  </div>

  {{-- content container --}}
  <div class="h-auto w-full p-4">
    <span class="flex flex-wrap text-justify xl:text-[17px]">{!! $exam->content !!}</span>
  </div>
  <hr class="w-full ">
  ...
  <div class="w-full px-4 py-2 ">
    ...
    <span class="text-blue-500 text-justify xl:text-[17px] ">{{$exam->links}}</span>
  </div>
</div>
